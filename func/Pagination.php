...
class Pagination
{
    ...
    public function get_link($uri):string
    {

        $prefix = preg_replace('/([?])page=(\d+)/', '', $_SERVER['REQUEST_URI']);

        return $prefix . '?page=' . $uri;
    ...
    public function get_html(): string
    {
        ...
        if ($this->total > $this->per_page) {
            return '<nav aria-label="Page navigation example"><ul class="pagination pagination-primary  justify-content-center">' . $start_page . $back . $pages_left . '<li class="page-item active"><a class="page-link">' . $this->current_page . '</a></li>' . $pages_right . $forward . $end_page . '</ul></nav>';
        } else {
            return false;
        }
    }
